...
USE TPM_FFTC, ONLY: FFTTMPBUFFER
#ifdef _OPENACC
USE OPENACC
#endif
...
CHARACTER(LEN=14)  :: CL_INITIAL_BUFFER
INTEGER(KIND=8) :: INITIAL_BUFFER_SZ
...
CL_INITIAL_BUFFER=' '
CALL EC_GETENV('ECTRANS_INITIAL_BUFFER', CL_INITIAL_BUFFER)
IF( CL_INITIAL_BUFFER /= ' ')THEN
  INITIAL_BUFFER_SZ = PARSE_I8(CL_INITIAL_BUFFER)
  CALL REALLOCATE_GROWING_ALLOCATION(GROWING_ALLOCATION, INITIAL_BUFFER_SZ)
ENDIF
CL_INITIAL_BUFFER=' '
CALL EC_GETENV('ECTRANS_FFT_BUFFER', CL_INITIAL_BUFFER)
IF( CL_INITIAL_BUFFER /= ' ')THEN
  INITIAL_BUFFER_SZ = PARSE_I8(CL_INITIAL_BUFFER)
  ALLOCATE(FFTTMPBUFFER(INITIAL_BUFFER_SZ))
  !$ACC ENTER DATA CREATE(FFTTMPBUFFER)
ENDIF
...
!endif INTERFACE
CONTAINS
FUNCTION PARSE_I8(STR)
  IMPLICIT NONE
  CHARACTER(LEN=*), INTENT(IN) :: STR
  INTEGER(KIND=8) :: PARSE_I8
  INTEGER :: I
  PARSE_I8 = 0
  DO I = 1, LEN(STR)
    IF (STR(I:I) >= '0' .AND. STR(I:I) <= '9' ) THEN
      PARSE_I8 = PARSE_I8 * 10 + IACHAR(STR(I:I)) - IACHAR('0')
    ELSE
      EXIT
    END IF
  END DO
