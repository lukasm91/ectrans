...
SUBROUTINE SETUP_TRANS(KSMAX,KDGL,KDLON,KLOEN,LDSPLIT,LDNONHYD,PSTRET,&
...
LOGICAL   ,OPTIONAL,INTENT(IN) :: LDNONHYD
...
INTEGER(KIND=JPIM) :: JMLOC, KM, ILA, ILS, IRET, KMLOC, KDGLU, JK, i, J
...
INTEGER(KIND=8) :: IWIDTH, IHEIGHT
INTEGER :: IPITCH_DST, IPITCH_SRC
INTEGER(KIND=CUDA_STREAM_KIND) :: STREAM
TYPE(C_PTR) :: HSTPTR
REAL(KIND=JPRB), POINTER :: RPNMA0(:,:), RPNMS0(:,:)
...
!$ACC ENTER DATA CREATE(ZAA,ZAS)
write(0,'("ZAA: ", I, " B")') size(ZAA,KIND=8)*sizeof(ZAA(1))
write(0,'("ZAS: ", I, " B")') size(ZAS,KIND=8)*sizeof(ZAS(1))
...
DO JMLOC=1,D%NUMP
  ...
  IF (KM /= 0) THEN
    ...
    !$ACC HOST_DATA USE_DEVICE(LOCAL_ARR)
    DEVPTR = C_DEVLOC(LOCAL_ARR)
    !$ACC END HOST_DATA
    IPITCH_DST = D%LEGENDRE_MATRIX_STRIDES(JMLOC)*SIZEOF(ZAA(1))
    HSTPTR = C_LOC(S%FA(JMLOC)%RPNMA)
    IPITCH_SRC = SIZE(S%FA(JMLOC)%RPNMA,1)*SIZEOF(ZAA(1))
    IWIDTH = KDGLU*SIZEOF(ZAA(1))
    IHEIGHT = ILA
    ...
    DEVPTR = C_DEVLOC(LOCAL_ARR)
    !$ACC END HOST_DATA
    IPITCH_DST = D%LEGENDRE_MATRIX_STRIDES(JMLOC)*SIZEOF(ZAS(1))
    HSTPTR = C_LOC(S%FA(JMLOC)%RPNMS)
    IPITCH_SRC = SIZE(S%FA(JMLOC)%RPNMS,1)*SIZEOF(ZAS(1))
    IWIDTH = KDGLU*SIZEOF(ZAS(1))
    IHEIGHT = ILS
    IRET = CUDAMEMCPY2DASYNC(DEVPTR, IPITCH_DST, &
                           & HSTPTR, IPITCH_SRC, &
                           & IWIDTH, IHEIGHT, &
  ...
  ELSE
    ...
    RPNMA0 => S%FA(JMLOC)%RPNMA(1:KDGLU,1:ILA)
    RPNMS0 => S%FA(JMLOC)%RPNMS(1:KDGLU,1:ILS)
    !$ACC ENTER DATA CREATE(ZAA0,ZAS0)
    !$ACC KERNELS DEFAULT(PRESENT) ASYNC(1)
    ZAA0(:,:) = 0
    ZAS0(:,:) = 0
    !$ACC END KERNELS
    !$ACC KERNELS DEFAULT(PRESENT) COPYIN(RPNMA0,RPNMS0) ASYNC(1)
    ZAA0(1:KDGLU,1:ILA)=RPNMA0(1:KDGLU,1:ILA)
    ZAS0(1:KDGLU,1:ILS)=RPNMS0(1:KDGLU,1:ILS)
    !$ACC END KERNELS
    write(0,'("ZAA0: ", I, " B")') size(ZAA0)*sizeof(ZAA0(1,1))
    write(0,'("ZAS0: ", I, " B")') size(ZAS0)*sizeof(ZAS0(1,1))
  ENDIF
ENDDO
!$ACC WAIT(1)

ALLOCATE(ZEPSNM(d%nump,0:R%NTMAX+2))
write(0,'("ZEPSNM: ", I, " B")') size(ZEPSNM)*sizeof(ZEPSNM(1, 1))
ZEPSNM = 0._JPRBT
CALL PREPSNM
