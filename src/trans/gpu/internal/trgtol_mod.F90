...
MODULE TRGTOL_MOD
  ...
  TYPE TRGTOL_HANDLE
    ...
    ! If USE_DIRECT_COPY_FOR_SELF, we must have all three buffers at the same time
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFR_AND_REEL

    ! Otherwise we can keep them separate
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFR, HREEL
  END TYPE
CONTAINS
  FUNCTION PREPARE_TRGTOL(ALLOCATOR,KF_GP,KF_FS) RESULT(HTRGTOL)
    ...
    IF (NPROC == 1) THEN
      HTRGTOL%HREEL = RESERVE(ALLOCATOR, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY), "HTRGTOL%HREEL")
    ELSEIF (USE_DIRECT_COPY_FOR_SELF) THEN
      HTRGTOL%HCOMBUFS = RESERVE(ALLOCATOR, 1_JPIB*KF_GP*D%NGPTOT*SIZEOF(DUMMY), "HTRGTOL%HCOMBUFS")
      NELEM = 0
      NELEM = NELEM + 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY) ! ZCOMBUFR
      NELEM = NELEM + 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY) ! PREEL_REAL
      HTRGTOL%HCOMBUFR_AND_REEL = RESERVE(ALLOCATOR, NELEM, "HTRGTOL%HCOMBUFR_AND_REEL")
    ELSE
      HTRGTOL%HCOMBUFS = RESERVE(ALLOCATOR, 1_JPIB*KF_GP*D%NGPTOT*SIZEOF(DUMMY), "HTRGTOL%HCOMBUFS")
      HTRGTOL%HCOMBUFR = RESERVE(ALLOCATOR, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY), "HTRGTOL%HCOMBUFR")
      HTRGTOL%HREEL = RESERVE(ALLOCATOR, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY), "HTRGTOL%HREEL")
    ENDIF
  END FUNCTION
  ...
  SUBROUTINE TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
    ...
    INTEGER(KIND=JPIM) :: IFIRSTLAT, IGL, IGLL, ILAST,&
                 ...
                 &ISEND_COUNTS,INS,INR,JKL, PBOUND, IERROR, ILOCAL_LAT
    ...
    IF (NPROC == 1) THEN
      CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HREEL),&
          & 1_JPIB, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(PREEL_REAL(1)))
    ELSEIF (USE_DIRECT_COPY_FOR_SELF) THEN
      CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFR_AND_REEL),&
          & 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(PREEL_REAL(1))+1, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(PREEL_REAL(1)))
    ELSE
      CALL ASSIGN_PTR(PREEL_REAL, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HREEL),&
          & 1_JPIB, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(PREEL_REAL(1)))
    ENDIF
    ...
    IF (LDUMP) THEN
      DO JROC=1,NPROC
        IF(ISENDTOT(JROC) > 0 .AND. JROC /= MYPROC) THEN
          WRITE(1000+MYPROC,*) "TRGTOL SEND", JROC, ISENDTOT(JROC)
        ENDIF
      ENDDO
      DO JROC=1,NPROC
        IF(IRECVTOT(JROC) > 0 .AND. JROC /= MYPROC) THEN
          WRITE(1000+MYPROC,*) "TRGTOL RECV", JROC, IRECVTOT(JROC)
        ENDIF
      ENDDO
      FLUSH(1000+MYPROC)
      LDUMP = .FALSE.
    ...
    IF (IRECV_COUNTS > 0) THEN
      ...
      ELSE
        CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFR),&
            & 1_JPIB, ICOMBUFR_OFFSET(NPROC+1)*SIZEOF(ZCOMBUFR(1)))
      ENDIF
    ENDIF
    ...
    CALL MPI_IALLTOALLV(ZCOMBUFS,ISENDTOT_MPI,ICOMBUFS_OFFSET_MPI,TRGTOL_DTYPE, &
                      & ZCOMBUFR,IRECVTOT_MPI,ICOMBUFR_OFFSET_MPI,TRGTOL_DTYPE, &
    ...
    ELSE IF((USE_DIRECT_COPY_FOR_SELF .OR. NPROC == 1) .AND. ISENDTOT(MYPROC) > 0 )THEN
      ...
      IF(ALLOCATED(PGP_DEV)) THEN
#ifdef HUGE_ARRAYS
        !$ACC PARALLEL LOOP GANG DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS,IOFF) ASYNC(1)
        DO JFLD=1,KF_FS
          !$ACC LOOP VECTOR
          DO JL=1,ISEND_WSET_SIZE_V
#else
    ...
    DO INR=1,IRECV_COUNTS
      ...
      ICOMBUFR_OFFSET_V = ICOMBUFR_OFFSET(IPROC)
