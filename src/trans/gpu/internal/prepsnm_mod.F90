...
#endif
        DO JN=0,KM-1
           FG%ZEPSNM(KMLOC,JN) = 0.0_JPRBT
        ENDDO
     ENDIF
 
     DO JN=KM,R%NTMAX+2
        FG%ZEPSNM(KMLOC,JN) = REAL(F%REPSNM(D%NPMT(KM)+KMLOC-KM+JN),JPRBT)
     ENDDO
...
SUBROUTINE PREPSNM_CPU(KM,KMLOC,PEPSNM)
