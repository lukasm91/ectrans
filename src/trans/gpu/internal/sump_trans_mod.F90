...
INTEGER(KIND=JPIM) :: IGPTOT,IMEDIAP,IRESTM,JA,JB,IOFF,KMLOC,KM
INTEGER(KIND=JPIB) :: OFFSET1,OFFSET2,OFFSET3
...
ALLOCATE(D%OFFSETS_GEMM_MATRIX(D%NUMP+1))
ALLOCATE(D%LEGENDRE_MATRIX_STRIDES(D%NUMP))
...
D%OFFSETS_GEMM_MATRIX(D%NUMP+1) = OFFSET3
