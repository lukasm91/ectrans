...
CONTAINS
  FUNCTION PREPARE_TRMTOL_PACK(ALLOCATOR,KF_LEG) RESULT(HTRMTOL_PACK)
    ...
    IALLOC_SZ = 2_JPIB*D%NLENGT1B*KF_LEG*SIZEOF(ZPRBT_DUMMY)
  ...
  SUBROUTINE TRMTOL_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,KF_LEG)
    ...
    USE TPM_DISTR, ONLY : D,D_NUMP,D_MYMS,D_NPNTGTB1,D_OFFSETS_GEMM1
    ...
    CALL ASSIGN_PTR(FOUBUF_IN, GET_ALLOCATION(ALLOCATOR, HTRMTOL_PACK%HFOUBUF_IN),&
      & 1_JPIB, 2_JPIB*D%NLENGT1B*KF_LEG*SIZEOF(FOUBUF_IN(1)))
...
#ifdef HUGE_ARRAYS
    !$ACC PARALLEL LOOP SEQ DEFAULT(NONE) PRIVATE(KM,ISL,IGLS,OFFSET1,OFFSET2,ZAOA,ZSOA) ASYNC(1)
    DO KMLOC=1,D_NUMP
      !$ACC LOOP COLLAPSE(2)
      DO JGL=1,R_NDGNH
        DO JK=1,2*KF_LEG
  ...
  FUNCTION PREPARE_TRMTOL_UNPACK(ALLOCATOR,KF_FS) RESULT(HTRMTOL_UNPACK)
    ...
    HTRMTOL_UNPACK%HREEL = RESERVE(ALLOCATOR, 1_JPIB*D%NLENGTF*KF_FS*SIZEOF(DUMMY), "HTRMTOL_UNPACK%HREEL")
