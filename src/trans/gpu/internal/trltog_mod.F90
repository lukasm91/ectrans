...
MODULE TRLTOG_MOD
  ...
  LOGICAL :: USE_DIRECT_COPY_FOR_SELF = .FALSE.
  ...
  SUBROUTINE TRLTOG_CUDAAWARE(ALLOCATOR,HTRLTOG,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,KPTRGP,&
    ...
    IF (IRECV_COUNTS > 0) THEN
      IF (USE_DIRECT_COPY_FOR_SELF) THEN
        CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR_AND_COMBUFS),&
            & 1_JPIB, ICOMBUFR_OFFSET(IRECV_COUNTS+1)*SIZEOF(ZCOMBUFR(1)))
      ELSE
    ...
    IF (ISEND_COUNTS > 0) THEN
      ...
      ELSE
        CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFS),&
            & 1_JPIB, ICOMBUFS_OFFSET(ISEND_COUNTS+1)*SIZEOF(ZCOMBUFS(1)))
      ENDIF
    ENDIF
    ...
    DO INS=1,ISEND_COUNTS
      ...
      IF (ISEND /= MYPROC) THEN
        IR=IR+1
        IF (INT(ISENDTOT(ISEND),KIND=JPIM) /= INT(ISENDTOT(ISEND),KIND=JPIB)) &
          & CALL MPL_ABORT("Overflow in trltog")
        CALL MPI_ISEND(ZCOMBUFS(ICOMBUFS_OFFSET(INS)+1:ICOMBUFS_OFFSET(INS+1)),INT(ISENDTOT(ISEND),JPIM), &
    ...
    IF (.NOT. USE_DIRECT_COPY_FOR_SELF .AND. NPROC > 1 .AND. ISENDTOT(MYPROC) > 0) THEN
       INR1 = FINDLOC(IRECV_TO_PROC, MYPROC)
       INS1 = FINDLOC(ISEND_TO_PROC, MYPROC)
       PRINT *, "INR", INR1(1), "INS", INS1(1)
       !$ACC KERNELS ASYNC(1)
       ZCOMBUFR(ICOMBUFR_OFFSET(INR1(1))+1:ICOMBUFR_OFFSET(INR1(1)+1)) = ZCOMBUFS(ICOMBUFS_OFFSET(INS1(1))+1:ICOMBUFS_OFFSET(INS1(1)+1))
       !$ACC END KERNELS
    ENDIF
