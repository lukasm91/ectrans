...
MODULE TRLTOG_MOD
  ...
  LOGICAL :: USE_DIRECT_COPY_FOR_SELF = .TRUE.

  TYPE TRLTOG_HANDLE
    ! If USE_DIRECT_COPY_FOR_SELF, we must have all buffers at the same time
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFR_AND_COMBUFS

    !Otherwise we can keep them separate
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFR
    TYPE(ALLOCATION_RESERVATION_HANDLE) :: HCOMBUFS
  END TYPE
CONTAINS
  FUNCTION PREPARE_TRLTOG(ALLOCATOR,KF_FS,KF_GP) RESULT(HTRLTOG)
    ...
    ELSEIF (USE_DIRECT_COPY_FOR_SELF) THEN
      NELEM = 0
      NELEM = NELEM + ALIGN(1_JPIB*KF_GP*D%NGPTOT*SIZEOF(DUMMY),128) ! ZCOMBUFR
      NELEM = NELEM + ALIGN(1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY),128) !ZCOMBUFS upper obund
      HTRLTOG%HCOMBUFR_AND_COMBUFS = RESERVE(ALLOCATOR, NELEM, "HTRLTOG%HCOMBUFR_AND_COMBUFS")
    ELSE
      HTRLTOG%HCOMBUFS = RESERVE(ALLOCATOR, 1_JPIB*KF_FS*D%NLENGTF*SIZEOF(DUMMY), "HTRLTOG%HCOMBUFS")
      HTRLTOG%HCOMBUFR = RESERVE(ALLOCATOR, 1_JPIB*KF_GP*D%NGPTOT*SIZEOF(DUMMY), "HTRLTOG%HCOMBUFR")
    ENDIF
  END FUNCTION
  ...
  SUBROUTINE TRLTOG_CUDAAWARE(ALLOCATOR,HTRLTOG,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,KPTRGP,&
    ...
    INTEGER(KIND=JPIM) :: IFIRSTLAT, ILASTLAT, IFLD, IGL, IGLL,&
                 ...
                 &ILOCAL_LAT, ISEND_COUNTS, IRECV_COUNTS, IERROR, II, ILEN, IBUFLENS, IBUFLENR, &
                 &JBLK, ILAT_STRIP
    INTEGER(KIND=JPIB) :: IPOS
    INTEGER(KIND=JPIM) :: INS1(1), INR1(1)
    ...
    INTEGER(KIND=JPIB) :: ICOMBUFS_OFFSET(NPROC+1),ICOMBUFR_OFFSET(NPROC+1)
    INTEGER(KIND=JPIM) :: ICOMBUFS_OFFSET_MPI(NPROC+1),ICOMBUFR_OFFSET_MPI(NPROC+1)
    ...
    IF((USE_DIRECT_COPY_FOR_SELF .OR. NPROC ==1 ) .AND. ISENDTOT(MYPROC) > 0) THEN
      ...
      IF (ALLOCATED(PGP_DEV)) THEN
#ifdef HUGE_ARRAYS
        !$ACC PARALLEL LOOP GANG DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,IPOS) ASYNC(1)
        DO JFLD=1,KF_FS
          !$ACC LOOP VECTOR
        ...
        DO JFLD=1,KF_FS
          DO JL=1,IRECV_WSET_SIZE_V
            ...
            PGP_DEV(JK,IFLD,JBLK) = PREEL_REAL(IPOS)
          ENDDO
        ENDDO
      ELSE
#ifdef HUGE_ARRAYS
        ...
        DO JFLD=1,KF_FS
          DO JL=1,IRECV_WSET_SIZE_V
            ...
            IF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_UV) THEN
              PGPUV_DEV(JK,IGP_OFFSETS(IFLD,3),IGP_OFFSETS(IFLD,2),JBLK) = PREEL_REAL(IPOS)
            ELSEIF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_GP2) THEN
              PGP2_DEV(JK,IGP_OFFSETS(IFLD,2),JBLK)=PREEL_REAL(IPOS)
            ELSEIF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_GP3A) THEN
              PGP3A_DEV(JK,IGP_OFFSETS(IFLD,3),IGP_OFFSETS(IFLD,2),JBLK) = PREEL_REAL(IPOS)
            ELSEIF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_GP3B) THEN
              PGP3B_DEV(JK,IGP_OFFSETS(IFLD,3),IGP_OFFSETS(IFLD,2),JBLK) = PREEL_REAL(IPOS)
            ENDIF
          ENDDO
        ENDDO
      ENDIF
    ...
    IRECV_COUNTS   = 0
    DO JROC=1,NPROC
      IF((NPROC > 1 .AND. .NOT. USE_DIRECT_COPY_FOR_SELF) .OR. JROC /= MYPROC) THEN
    ...
    IF (LDUMP) THEN
      DO JROC=1,NPROC
        IF(ISENDTOT(JROC) > 0 .AND. JROC /= MYPROC) THEN
          WRITE(1000+MYPROC,*) "TRLTOG SEND", JROC, ISENDTOT(JROC)
        ENDIF
      ENDDO
      DO JROC=1,NPROC
        IF(IRECVTOT(JROC) > 0 .AND. JROC /= MYPROC) THEN
          WRITE(1000+MYPROC,*) "TRLTOG RECV", JROC, IRECVTOT(JROC)
        ENDIF
      ENDDO
      FLUSH(1000+MYPROC)
      LDUMP = .FALSE.
    ...
    ICOMBUFS_OFFSET(1) = 0
    DO JROC=1,NPROC
      ICOMBUFS_OFFSET(JROC+1) = ICOMBUFS_OFFSET(JROC) + ISENDTOT(JROC)
    ENDDO
    ICOMBUFR_OFFSET(1) = 0
    DO JROC=1,NPROC
      ICOMBUFR_OFFSET(JROC+1) = ICOMBUFR_OFFSET(JROC) + IRECVTOT(JROC)
    ENDDO

    IF (IRECV_COUNTS > 0) THEN
      IF (USE_DIRECT_COPY_FOR_SELF) THEN
        CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR_AND_COMBUFS),&
            & 1_JPIB, ICOMBUFR_OFFSET(NPROC+1)*SIZEOF(ZCOMBUFR(1)))
      ELSE
        CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR),&
            & 1_JPIB, ICOMBUFR_OFFSET(NPROC+1)*SIZEOF(ZCOMBUFR(1)))
      ENDIF
    ENDIF
    IF (ISEND_COUNTS > 0) THEN
      IF (USE_DIRECT_COPY_FOR_SELF) THEN
        CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR_AND_COMBUFS),&
            & ALIGN(1_JPIB*KF_GP*D%NGPTOT*SIZEOF(ZCOMBUFR(1)),128)+1, &
            & ICOMBUFS_OFFSET(NPROC+1)*SIZEOF(ZCOMBUFS(1)))
      ELSE
        CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFS),&
            & 1_JPIB, ICOMBUFS_OFFSET(NPROC+1)*SIZEOF(ZCOMBUFS(1)))
      ENDIF
    ENDIF
    ...
    DO INS=1,ISEND_COUNTS
      ...
      IIN_TO_SEND_BUFR_V = IIN_TO_SEND_BUFR_OFFSET(ISEND)
      ICOMBUFS_OFFSET_V = ICOMBUFS_OFFSET(ISEND)
#ifdef HUGE_ARRAYS
      !$ACC PARALLEL LOOP GANG PRIVATE(IPOS) ASYNC(1)
      DO JFLD=1,KF_FS
        !$ACC LOOP VECTOR
        DO JL=1,ILEN
#else
    ...
    IF (ANY(IRECVTOT_MPI /= IRECVTOT)) &
      & CALL MPL_ABORT("Overflow in trltog")
    ISENDTOT_MPI(MYPROC) = 0
    IRECVTOT_MPI(MYPROC) = 0
    ICOMBUFR_OFFSET_MPI = ICOMBUFR_OFFSET
    ICOMBUFS_OFFSET_MPI = ICOMBUFS_OFFSET
    CALL MPI_IALLTOALLV(ZCOMBUFS,ISENDTOT_MPI,ICOMBUFS_OFFSET_MPI,TRLTOG_DTYPE, &
                      & ZCOMBUFR,IRECVTOT_MPI,ICOMBUFR_OFFSET_MPI,TRLTOG_DTYPE, &
                      & MPL_COMM_OML(OML_MY_THREAD()),IREQ,IERROR)
    !$ACC END HOST_DATA

    IF (.NOT. USE_DIRECT_COPY_FOR_SELF .AND. NPROC > 1 .AND. ISENDTOT(MYPROC) > 0) THEN
       !$ACC KERNELS ASYNC(1)
       ZCOMBUFR(ICOMBUFR_OFFSET(MYPROC)+1:ICOMBUFR_OFFSET(MYPROC+1)) &
               & = ZCOMBUFS(ICOMBUFS_OFFSET(MYPROC)+1:ICOMBUFS_OFFSET(MYPROC+1))
       !$ACC END KERNELS
    ENDIF

    CALL MPL_WAIT(KREQUEST=IREQ, &
    ...
    DO INR=1,IRECV_COUNTS
      ...
      ICOMBUFR_OFFSET_V = ICOMBUFR_OFFSET(IRECV)
      ...
      IF (ALLOCATED(PGP_DEV)) THEN
        ...
        DO JFLD=1,IRECV_FIELD_COUNT_V
          DO JL=1,IRECV_WSET_SIZE_V
            ...
            PGP_DEV(JK,IFLD,JBLK) = ZCOMBUFR(JI)
          ENDDO
        ENDDO
      ELSE
        !$ACC PARALLEL LOOP COLLAPSE(2) DEFAULT(NONE) PRIVATE(JK,JBLK,IFLD,JI) ASYNC(1)
        DO JFLD=1,IRECV_FIELD_COUNT_V
          DO JL=1,IRECV_WSET_SIZE_V
            ...
            JI = ICOMBUFR_OFFSET_V+(JFLD-1)*IRECV_WSET_SIZE_V+JL
            IF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_UV) THEN
              PGPUV_DEV(JK,IGP_OFFSETS(IFLD,3),IGP_OFFSETS(IFLD,2),JBLK) = ZCOMBUFR(JI)
            ELSEIF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_GP2) THEN
              PGP2_DEV(JK,IGP_OFFSETS(IFLD,2),JBLK) = ZCOMBUFR(JI)
            ELSEIF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_GP3A) THEN
              PGP3A_DEV(JK,IGP_OFFSETS(IFLD,3),IGP_OFFSETS(IFLD,2),JBLK) = ZCOMBUFR(JI)
            ELSEIF(IGP_OFFSETS(IFLD,1) == IGP_OFFSETS_GP3B) THEN
              PGP3B_DEV(JK,IGP_OFFSETS(IFLD,3),IGP_OFFSETS(IFLD,2),JBLK) = ZCOMBUFR(JI)
            ENDIF
          ENDDO
        ENDDO
      ENDIF
    ...
    ENDIF
    CALL GSTATS(422,0)
    IF (PRESENT(PGP))   CALL COPY(PGP,   PGP_DEV,   ND2H, QUEUE=1_ACC_HANDLE_KIND)
    IF (PRESENT(PGPUV)) CALL COPY(PGPUV, PGPUV_DEV, ND2H, QUEUE=1_ACC_HANDLE_KIND)
    IF (PRESENT(PGP2))  CALL COPY(PGP2,  PGP2_DEV,  ND2H, QUEUE=1_ACC_HANDLE_KIND)
    IF (PRESENT(PGP3A)) CALL COPY(PGP3A, PGP3A_DEV, ND2H, QUEUE=1_ACC_HANDLE_KIND)
    IF (PRESENT(PGP3B)) CALL COPY(PGP3B, PGP3B_DEV, ND2H, QUEUE=1_ACC_HANDLE_KIND)
    !$ACC END DATA ! PGP3B
    !$ACC END DATA ! PGP3A
    !$ACC END DATA ! PGP2
    !$ACC END DATA ! PGPUV
