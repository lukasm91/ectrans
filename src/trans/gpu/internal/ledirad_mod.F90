...
!*       1. ANTISYMMETRIC PART.
  ...
  ENDDO
    ...
    CALL GEMM('N','N',KDGLU,KIFC,ILA,1.0_JPRB,S%FA(KMLOC)%RPNMA,KDGLU,ZCA,ILA,0._JPRB,ZB,KDGLU)
  ...
  DO JK=1,KFC,ISKIP
    IFLD=IFLD+1
    DO J=1,KDGLU
      PAIA(JK,ISL+J-1) = ZB(J,IFLD)*REAL(F%RW(ISL+J-1),JPRB)
    ENDDO
  ENDDO
...
!*       1.3      SYMMETRIC PART.
  ...
  ENDDO
    ...
    CALL GEMM('N','N',KDGLU,KIFC,ILS,1.0_JPRD,S%FA(KMLOC)%RPNMS,KDGLU,ZCS,ILS,0._JPRD,ZB,KDGLU)
  ...
  DO JK=1,KFC,ISKIP
    IFLD=IFLD+1
    DO J=1,KDGLU
      PSIA(JK,ISL+J-1) = ZB(J,IFLD)*REAL(F%RW(ISL+J-1),JPRB)
    ENDDO
  ENDDO
