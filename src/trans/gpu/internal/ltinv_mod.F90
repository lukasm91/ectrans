...
CONTAINS
  FUNCTION PREPARE_LTINV(ALLOCATOR,KF_UV,KF_SCALARS,LVORGP,LDIVGP,LSCDERS) RESULT(HLTINV)
    ...
    IPIA_SZ = ALIGN(2_JPIB*IF_READIN*(R%NSMAX+4)*D%NUMP*C_SIZEOF(ZPRBT_DUMMY),128)
  ...
  USE IEEE_ARITHMETIC
    ...
    ! PIA
    IALLOC_SZ = ALIGN(2_JPIB*IF_READIN*(R%NSMAX+4)*D%NUMP*C_SIZEOF(PIA_L(1)),128)
    CALL ASSIGN_PTR(PIA_L, GET_ALLOCATION(ALLOCATOR, HLTINV%HPIA_AND_IN),&
        & IALLOC_POS, IALLOC_SZ)
    CALL C_F_POINTER(C_LOC(PIA_L), PIA, (/ 2*IF_READIN, R%NSMAX+4, D%NUMP /))
    ...
    !$ACC& DEFAULT(NONE) PRESENT(D,PIA,R) &
    !$ACC& ASYNC(1)
    DO KMLOC=1,D%NUMP
    DO JK=1,2*IF_LEG
       KM =  D%MYMS(KMLOC)
       IA  = 1+MOD(R%NSMAX-KM+2,2)
       PIA(2*(IF_READIN-IF_LEG)+JK,1,KMLOC) = 0
       DO J=0,(R%NSMAX-KM+2)/2-1
        TMP = PIA(2*(IF_READIN-IF_LEG)+JK,R%NSMAX-KM+3-J,KMLOC)
        PIA(2*(IF_READIN-IF_LEG)+JK,R%NSMAX-KM+3-J,KMLOC) = PIA(2*(IF_READIN-IF_LEG)+JK,2+J,KMLOC)
        PIA(2*(IF_READIN-IF_LEG)+JK,J+2,KMLOC) = TMP
       ENDDO
       DO J=R%NSMAX-KM+4,R%NSMAX+4
        PIA(2*(IF_READIN-IF_LEG)+JK,J,KMLOC) = 0
       ENDDO
      ENDDO
    ENDDO
